...
def build():
    ...
    notes(s, "1. Este es el diagrama de los autores, y son los mismos cuatro pasos que acabamos de "
             "recorrer, ahora con las letras de ellos. Lo leo de izquierda a derecha.\n"
             "\n"
             "2. A la izquierda, la lámina se corta en parches y cada parche pasa por el encoder, "
             "que en nuestro caso es CONCH, y sale convertido en un vector de características. En la "
             "figura ese vector es equis sub i, hay uno por parche, y la lámina entera es el "
             "conjunto que va de equis sub uno hasta equis sub ene.\n"
             "\n"
             "3. En un modelo de múltiples instancias corriente, cada uno de esos vectores entraría "
             "por una sola matriz lineal, la doble ve del diagrama, que lo proyecta al espacio "
             "interno del agregador. Ese es el único lugar donde MAMMOTH interviene: reemplaza esa "
             "matriz. El encoder de la izquierda y el agregador de la derecha quedan intactos.\n"
             "\n"
             "4. Lo primero que hace es partir esa proyección en cabezas. La salida se corta en "
             "tramos y a cada tramo la figura lo llama equis con barra, la porción del parche que ve "
             "cada cabeza. Son subespacios aprendidos, como en la atención de múltiples cabezas, y "
             "no corresponden a textura, forma ni color.\n"
             "\n"
             "5. Cada cabeza corre su propia mezcla de expertos en paralelo, que son los bloques "
             "rotulados como mezcla de expertos. Dentro de cada experto están los slots, que son "
             "prototipos aprendidos. El ruteo compara cada parche con cada prototipo mediante un "
             "producto interno, y sobre todos los parches de la lámina se aplica una softmax que "
             "decide cuánto aporta cada parche a cada slot. Con esos pesos se arma el promedio "
             "ponderado que la figura llama promedio pesado, y ese promedio es lo que llena el slot. "
             "El slot termina representando un fenotipo, un tipo de tejido.\n"
             "\n"
             "6. Cada slot pasa después por la transformación de su experto, que en la figura es fi "
             "por la matriz de rango bajo seguida de una no linealidad. Esa factorización es lo que "
             "permite tener treinta expertos sin gastar más parámetros que la matriz original.\n"
             "\n"
             "7. A la derecha se concatenan las cabezas y sale la salida de MAMMOTH, que alimenta al "
             "agregador. Acá el agregador es CLAM: su atención junta los parches en un único vector "
             "de lámina y el clasificador entrega el diagnóstico. Un detalle de lectura, porque la "
             "figura dibuja como salida los propios slots: en nuestra configuración base una segunda "
             "softmax los recombina y devuelve un vector por parche, con la misma forma que entró, "
             "así el reemplazo de la capa lineal es directo.\n"
             "\n"
             "8. Suele aparecer acá una pregunta: por qué esto es una mezcla de expertos y no un "
             "producto de expertos. La respuesta está en cómo se combinan. Acá se combinan sumando: "
             "las dos softmax reparten pesos que suman uno, así que la salida es un promedio, una "
             "combinación convexa, y eso se entrena directo y es estable. Un producto multiplicaría "
             "las distribuciones de los expertos, como si cada uno tuviera derecho a veto, y "
             "necesitaría un normalizador que es intratable; además describe una distribución de "
             "probabilidad, y lo que hace falta acá es transformar características. Aclaro que el "
             "paper no discute el producto de expertos: la comparación es un razonamiento de "
             "arquitectura, no una cita.\n"
             "\n"
             "9. En el diagrama se cruzan tres números, y la lámina siguiente los ordena: dieciséis "
             "cabezas, treinta expertos y diez slots por experto.")
    ...
    notes(s, "1. La duda que quedó dando vueltas la vez pasada es si dieciséis cabezas quieren decir "
             "dieciséis expertos. No: cabezas y expertos son ejes distintos, y el esquema de la "
             "izquierda los muestra uno dentro del otro.\n"
             "\n"
             "2. Arriba entra el parche. Debajo está la cabeza, y hay dieciséis en paralelo, cada "
             "una mirando su tramo del parche. Dentro de cada cabeza están los treinta expertos, y "
             "dentro de cada experto, las diez celdas dibujadas, que son sus prototipos. Treinta "
             "expertos por diez prototipos dan los trescientos slots.\n"
             "\n"
             "3. De ahí sale la línea del medio: las combinaciones de cabeza y experto son "
             "cuatrocientas ochenta. Conviene atar ese número al de la lámina anterior para que no "
             "parezcan dos cuentas que compiten. Cada una de esas combinaciones aporta diez slots, "
             "así que cada parche se compara contra los trescientos prototipos en cada una de sus "
             "dieciséis cabezas, y eso da los cuatro mil ochocientos parecidos que vimos recién. "
             "Cuatrocientos ochenta cuenta pares de cabeza y experto; cuatro mil ochocientos cuenta "
             "comparaciones por parche.\n"
             "\n"
             "4. La tabla de la derecha resume los tres ejes: dieciséis cabezas, que son subespacios "
             "del parche; treinta expertos, que se especializan por morfología; diez slots por "
             "experto, que son sus prototipos; y trescientos slots en total, que es el producto de "
             "los dos últimos.\n"
             "\n"
             "5. Abajo quedan las dos softmax, que son las dos direcciones sobre los mismos "
             "parecidos. El reparto hacia los slots va sobre los parches y llena cada slot. La "
             "recombinación va sobre los trescientos slots y rearma cada parche. Esta segunda "
             "conviene retenerla, porque es la medida con la que trabaja toda la última parte de "
             "hoy.\n"
             "\n"
             "6. Con los tres ejes ordenados, queda una variante del método que conviene nombrar "
             "antes de pasar a lo que se midió.")
    ...
    notes(s, "1. La variante se llama keep slots, y lo importante es dónde cambia, porque hasta la "
             "mitad del diagrama todo es idéntico. El tronco de arriba es el que ya recorrimos: "
             "entran las características, viene el ruteo, los expertos transforman y las cabezas se "
             "concatenan. Ahí quedan los trescientos slots.\n"
             "\n"
             "2. La bifurcación es literalmente una línea de código, y está escrita bajo cada rama.\n"
             "\n"
             "3. A la izquierda, la configuración base, que es la que usamos en todo lo de hoy. La "
             "segunda softmax recombina los trescientos slots y reconstruye un vector por parche, "
             "así que CLAM agrega sobre los parches, igual que con la capa lineal original.\n"
             "\n"
             "4. A la derecha, la variante. Se omite esa recombinación y la salida son los "
             "trescientos slots, de modo que CLAM ya no agrega sobre los parches sino sobre "
             "trescientos elementos.\n"
             "\n"
             "5. La frase de abajo es lo que importa para hoy. La cabeza de CLAM y la función de "
             "pérdida son las mismas en las dos ramas, y lo único que cambia es qué se agrega. Como "
             "todo el análisis que viene se calcula sobre el tronco, antes de la bifurcación, vale "
             "igual para las dos.\n"
             "\n"
             "6. Con el mecanismo cerrado, pasamos a mirar qué aprende por dentro.")
    ...
    notes(s, "Empieza la segunda parte. La pregunta ahora es qué región de la lámina concentra cada "
             "experto y qué morfología hay ahí, mirado sobre las propias láminas del proyecto. Es un "
             "análisis posterior al entrenamiento, hecho en CPU sobre un modelo ya entrenado: no "
             "reentrena nada y no toca ninguna métrica.")
    ...
    notes(s, "1. Las dos vistas de la lámina cuentan una sola cosa, así que las leo juntas.\n"
             "\n"
             "2. A la izquierda, cada uno de los treinta cuadros es la misma lámina pintada según "
             "cuánto el ruteo manda cada parche a ese experto: rojo mucho, azul casi nada. Lo que se "
             "ve es que los treinta encienden zonas distintas, o sea que la capa lineal única quedó "
             "reemplazada por especialistas que miran regiones diferentes del tejido.\n"
             "\n"
             "3. Una precisión sobre el color, que está en el pie: está normalizado por percentil "
             "dentro de cada experto. Sirve para ver qué regiones prefiere cada uno, no para decir "
             "cuál se usa más. Medido aparte, el uso sale casi uniforme, sin expertos apagados ni "
             "acaparadores.\n"
             "\n"
             "4. El mapa dice dónde, pero no qué tejido hay ahí. Eso lo responde la derecha: para "
             "cada experto se toman los parches que más rutea y se recortan a resolución alta. El "
             "experto ocho concentra nidos de epitelio tumoral, el veintiséis estroma fibroso y el "
             "tres, ductos.\n"
             "\n"
             "5. Dos aclaraciones sobre eso. La especialización emerge sin supervisión de tejido, "
             "porque nadie le indicó al modelo qué es estroma, y el paper validó lo mismo con "
             "patólogos. Y los nombres de tejido que uso son lectura visual nuestra, todavía sin "
             "revisión de un patólogo, que es una de las cosas que quiero pedir hoy.\n"
             "\n"
             "6. Que aparezcan expertos con más de un patrón es esperable, porque el ruteo es suave "
             "y reparte cada parche entre varios.\n"
             "\n"
             "7. La lámina siguiente pone a prueba si esta especialización se sostiene de una lámina "
             "a otra.")
    ...
    notes(s, "1. Esta es la prueba más fina de las tres y la que cierra esta parte.\n"
             "\n"
             "2. Los prototipos son parámetros del modelo, compartidos entre láminas, así que el "
             "experto ocho es el mismo experto en todas. Eso permite una prueba limpia: si su "
             "especialización es real, tiene que elegir la misma morfología en todas.\n"
             "\n"
             "3. En las dos grillas, cada fila es una de las cuatro láminas y cada columna un parche "
             "de los que más rutea ese experto. El ocho enciende epitelio en las cuatro y el "
             "veintiséis estroma en las cuatro, incluidas las dos negativas.\n"
             "\n"
             "4. El matiz de la palabra negativa importa, y está escrito abajo: acá significa "
             "carcinoma ductal in situ sin microcalcificación, no ausencia de tumor. Las negativas "
             "siguen siendo láminas de mama con epitelio y con estroma.\n"
             "\n"
             "5. La conclusión es que el experto detecta tejido, no clase. Que la lámina sea "
             "positiva se decide después, en la atención y en el clasificador.\n"
             "\n"
             "6. Y de ahí sale el punto de discusión, que es la franja de abajo: que los expertos "
             "separen bien los tejidos y aun así la métrica no se mueva respecto de CLAM indica que "
             "el cuello de botella no está en la primera capa, sino en el dato.\n"
             "\n"
             "7. Los límites de la prueba están dichos en la lámina: cuatro láminas, una tarea, una "
             "partición y etiquetas de tejido provisionales. Y son dos ejes distintos, conviene "
             "separarlos: que MAMMOTH no mejore la métrica está cerrado; qué aprende por dentro está "
             "abierto, y es lo que traigo hoy.\n"
             "\n"
             "8. Todo esto es una cala chica sobre cuatro láminas. Lo que viene es el trabajo nuevo, "
             "con entrenamiento pareado sobre tres tareas clínicas.")
    ...
    notes(s, "Hasta acá vimos el mecanismo por dentro y qué concentra cada experto sobre una cala "
             "chica. Esta parte es el trabajo nuevo: entrenar los dos modelos sobre exactamente las "
             "mismas particiones de tres tareas clínicas y después comparar, lámina por lámina, "
             "dónde pone la atención cada uno. La pregunta ya no es cuál mide mejor, sino en qué se "
             "parecen y en qué se diferencian por dentro cuando ven el mismo tejido.")
    ...
    takeaway_bar(s, "Candidato a réplica con más semillas antes de contarlo como mejora.",
                 t=4.62, size=12)
    notes(s, "1. Empiezo por el diseño, porque es lo que hace que los números signifiquen algo. Los "
             "dos modelos corrieron sobre exactamente las mismas particiones, y eso no se dio por "
             "supuesto: se comparó la firma de los identificadores de lámina del conjunto de prueba, "
             "partición por partición. Tres tareas, cinco particiones, dos brazos, treinta "
             "ejecuciones completas.\n"
             "\n"
             "2. La tabla se lee por la columna de la diferencia pareada, de arriba abajo. Esa "
             "columna es la resta entre los dos brazos dentro de cada partición, y al lado va en "
             "cuántas de las cinco esa resta salió a favor.\n"
             "\n"
             "3. En tipo histológico y en invasión linfovascular la diferencia es negativa y chica, "
             "y la desviación entre particiones iguala o supera a la media. Eso es lo que estaba "
             "anticipado por escrito antes de correr: no hay señal.\n"
             "\n"
             "4. La tercera fila es distinta. En carcinoma ductal in situ la diferencia es positiva "
             "en las cinco particiones, y el área bajo la curva se mueve en la misma dirección.\n"
             "\n"
             "5. Hay un detalle que importa y está en el recuadro: suben los dos recalls a la vez, "
             "el de la clase minoritaria y el de la mayoritaria. Cuando uno corre el punto de corte, "
             "sube uno y baja el otro. Acá eso no pasa, así que la mejora está en el ordenamiento y "
             "no en el umbral.\n"
             "\n"
             "6. Aun así freno antes de llamarlo mejora, por una razón de tamaño: son sesenta y "
             "cinco negativos en total, unos trece por partición, y además es una formulación de la "
             "tarea que es nueva. Queda como candidato a replicar con más semillas, no como "
             "resultado cerrado.\n"
             "\n"
             "7. Con las métricas puestas, la pregunta que sigue es dónde mira cada modelo.")
    ...
    takeaway_bar(s, "Que la mayor difusión aparezca donde MAMMOTH también mide mejor es "
                 ...
                 t=4.62, size=11.5)
    notes(s, "1. El resultado central de la parte nueva sale de esta lámina.\n"
             "\n"
             "2. Primero cómo se eligieron las láminas, que está en el subtítulo: siete, una por "
             "cada clase de cada tarea, todas del conjunto de prueba y además bien clasificadas por "
             "los dos modelos. Esa segunda condición es deliberada, porque si uno de los dos se "
             "equivoca la comparación de dónde mira queda contaminada por el error. Y la comparación "
             "es directa porque la variante hereda el mismo mecanismo de atención del modelo base: "
             "los dos mapas salen del mismo código.\n"
             "\n"
             "3. A la izquierda está una de las siete. Primero la atención de CLAM, después la de "
             "MAMMOTH y al costado la diferencia entre las dos, sobre la misma lámina.\n"
             "\n"
             "4. Los números de la derecha resumen las siete y dicen tres cosas. La primera es que "
             "coinciden en el mapa grueso: la correlación de rangos es alta, los dos ordenan el "
             "tejido de forma parecida y la región que importa es la misma. La segunda es que "
             "difieren en los picos: si uno mira el cinco por ciento de parches más atendidos el "
             "solapamiento es bajo, y en el uno por ciento superior es casi nulo. De ahí el título "
             "de la lámina. La tercera es que la entropía es mayor en MAMMOTH en seis de las siete "
             "láminas, o sea que reparte la atención donde CLAM la concentra.\n"
             "\n"
             "5. La franja de abajo es una advertencia deliberada. La mayor dispersión aparece justo "
             "en la tarea donde la variante también mide mejor, y da ganas de unir las dos cosas, "
             "pero con siete láminas eso es una hipótesis y no una explicación.\n"
             "\n"
             "6. Queda una última pregunta de esta parte: cuánta de la capacidad del método se usa "
             "de verdad.")
    ...
    notes(s, "1. Esta pregunta quedó abierta de la reunión anterior: cuántos expertos y cuántos "
             "slots usa realmente el modelo.\n"
             "\n"
             "2. Antes del número hay que precisar qué se mide, y está en el subtítulo. La medida es "
             "el peso de combinación, la segunda softmax, la que reparte cada parche entre los "
             "trescientos slots. No es el conteo de parches más atendidos, que es otra cosa "
             "distinta.\n"
             "\n"
             "3. Hay un detalle metodológico que explica la medida elegida. Como es una softmax, "
             "todos los slots reciben algo de peso y ninguno queda en cero, así que contar cuántos "
             "reciben peso daría siempre trescientos. Por eso se usa el número efectivo, que es la "
             "exponencial de la entropía: da el total si el reparto fuera perfectamente parejo y da "
             "uno si todo el peso cayera en un solo slot.\n"
             "\n"
             "4. Con esa medida, los expertos dan treinta de treinta. Se usan los treinta por igual "
             "y ninguno queda apagado, así que el número de expertos no está sobredimensionado.\n"
             "\n"
             "5. Los slots dan alrededor de ciento sesenta de trescientos, y varían bastante de una "
             "lámina a otra, entre noventa y ciento noventa y seis, siguiendo el tamaño de la lámina "
             "y no la tarea.\n"
             "\n"
             "6. La lectura práctica es la de abajo: si en algún momento hay que ajustar capacidad, "
             "el parámetro a tocar son los slots y no los expertos.\n"
             "\n"
             "7. Las dos láminas que siguen bajan esto al tejido: primero dónde se concentra cada "
             "slot y después cuántos aportan de verdad.")
    ...
    notes(s, "1. Esta lámina pone la segunda softmax sobre el tejido. Cada mapa toma un slot "
             "concreto, mira qué fracción de su peso le dedica cada parche y pinta esa fracción "
             "sobre la imagen.\n"
             "\n"
             "2. Cada fila es una tarea. A la izquierda está la lámina limpia como referencia y a la "
             "derecha los cuatro slots que más peso concentran en ella, en orden.\n"
             "\n"
             "3. El porcentaje que aparece sobre cada mapa es el peso de ese slot promediado sobre "
             "todos los parches, y se lee contra el reparto parejo, que sería un tercio de uno por "
             "ciento. Los slots que encabezan están entre nueve y quince veces por encima de lo que "
             "les tocaría si el ruteo fuera ciego.\n"
             "\n"
             "4. El quince por ciento del título es otra cosa y conviene separarla, porque se "
             "confunde. No es un parámetro del modelo, es un recorte de dibujo: se pintan solo los "
             "parches donde ese slot pesa más, el quince por ciento superior, y el resto queda "
             "transparente para que se vea el tejido debajo. Pintando todos los parches los mapas "
             "salen casi iguales entre sí y el contraste se pierde.\n"
             "\n"
             "5. La tabla del costado es el mismo ranking en números, y deja ver la cola: los cuatro "
             "primeros juntan algo más del diez por ciento del peso y los doscientos noventa y seis "
             "restantes se reparten el resto.\n"
             "\n"
             "6. Y hay un caso que quiero señalar, en la fila del medio. El slot que va primero y el "
             "que va cuarto son del mismo experto, el veintiocho. Si el experto fuera la unidad de "
             "morfología, esos dos mapas tendrían que parecerse, y no se parecen: la correlación "
             "espacial entre ellos es negativa, encienden regiones distintas de la misma lámina. Es "
             "lo mismo que muestra la figura del paper cuando rotula sus mapas por par de experto y "
             "slot, y encaja con que a nivel de experto el ruteo salga uniforme.\n"
             "\n"
             "7. Con la salvedad de siempre: qué tejido es cada región es lectura nuestra y todavía "
             "no tiene revisión de un patólogo.\n"
             "\n"
             "8. Viendo los mapas aparece sola la pregunta de la lámina siguiente: cuántos de los "
             "trescientos slots hacen falta.")
    ...
    notes(s, "1. Para responder cuántos slots hacen falta hay que fijar una cota, y el problema está "
             "en la tabla de la izquierda. Cortando en uno por ciento quedan veinticinco slots; "
             "cortando en cero coma cero uno por ciento quedan doscientos noventa y nueve. El número "
             "que uno reporta terminaría diciendo más sobre dónde puso el corte que sobre el modelo.\n"
             "\n"
             "2. La cota que elegimos es el reparto parejo, uno entre trescientos, cero coma tres "
             "tres por ciento. Tiene una ventaja sobre cualquier otra: no hay nada que elegir a "
             "mano. Un slot por debajo de eso recibe menos de lo que le tocaría si el ruteo fuera "
             "ciego, así que no está concentrando nada.\n"
             "\n"
             "3. Con esa cota, medida lámina por lámina, quedan entre sesenta y tres y noventa y "
             "seis slots por encima, y esos concentran alrededor de tres cuartas partes del peso "
             "total. Lo notable es que el recuento es parecido en las tres tareas.\n"
             "\n"
             "4. La última columna es el número efectivo de la lámina anterior, y da más alto, "
             "alrededor de ciento sesenta. No se contradicen, miden cosas distintas. Los ochenta y "
             "cinco son los que concentran. El número efectivo cuenta a cada slot en proporción a su "
             "peso, y sale más alto porque los doscientos quince de la cola, aunque cada uno pese "
             "menos que el reparto ciego, entre todos suman la cuarta parte que falta.\n"
             "\n"
             "5. Sobre la entropía en sí, que está explicada abajo, la idea es simple: mide cuán "
             "repartido está el peso. Si estuviera perfectamente repartido entre los trescientos, la "
             "exponencial daría trescientos exacto; si todo cayera en un solo slot, daría uno. "
             "Nuestro ciento sesenta dice que el reparto quedó a mitad de camino.\n"
             "\n"
             "6. Una advertencia sobre la dispersión de esa columna, porque se presta a confusión. "
             "No varía con la tarea, varía con el tamaño de la lámina, y las dos láminas de una "
             "misma tarea cubren casi todo el rango por sí solas. Cuantos menos parches tiene la "
             "lámina, menos morfología distinta hay que rutear. La tendencia tampoco es limpia, "
             "porque la lámina más grande de todas rompe el orden.\n"
             "\n"
             "7. La lectura práctica es la de abajo: hay margen para recortar el presupuesto de "
             "slots. Bajar de trescientos a doscientos setenta queda holgado por las dos medidas. "
             "Bajar a ciento cincuenta queda por encima de los que concentran y por debajo del "
             "número efectivo, así que desde ahí es una pregunta que se responde entrenando, no "
             "calculando.\n"
             "\n"
             "8. Con esto cierra la parte de arquitectura. Lo que queda es la línea de trabajo "
             "nueva.")
    ...
    notes(s, "Cerrado el capítulo de la arquitectura, donde cuatro ejes distintos no movieron la "
             "métrica, queda una idea que todavía no probamos y que sí trae información nueva: la "
             "escala a la que miramos el tejido. Esta parte es un piloto sobre microcalcificaciones, "
             "elegido porque son pocas láminas y la extracción cabe en un fin de semana, pero la "
             "misma idea sirve para cualquier tarea que dependa del contexto.")
    ...
    notes(s, "1. Empiezo por algo que sorprende. Uno pensaría que una microcalcificación, por ser un "
             "objeto chico, pide más aumento, y es lo contrario.\n"
             "\n"
             "2. La etiqueta que queremos predecir, que está arriba, no pregunta si hay una "
             "calcificación, sino en qué estructura vive: dentro de un carcinoma in situ, de un "
             "carcinoma invasor o de tejido no neoplásico. Eso es contexto, no detalle celular.\n"
             "\n"
             "3. Los tres tamaños de la izquierda lo dejan a la vista. La calcificación mide entre "
             "cincuenta y quinientas micras. La estructura que la aloja, el conducto o el lobulillo, "
             "mide entre medio milímetro y dos milímetros. Y el parche fino que extraemos hoy cubre "
             "cerca de cien micras: alcanza para la calcificación y no para el conducto que la "
             "rodea. Por eso lo que falta no es más aumento, es sumar una escala gruesa que traiga "
             "el contexto.\n"
             "\n"
             "4. A la derecha está un segundo hallazgo, que apareció al medir las láminas. Uno "
             "esperaría que un parche del mismo tamaño en píxeles cubriera lo mismo en las dos "
             "cohortes, y no. Con la resolución real de cada escáner, la cohorte pública está cerca "
             "de cuarenta aumentos y la privada cerca de veinte, aunque la etiqueta del archivo diga "
             "otra cosa. El mismo parche de doscientos cincuenta y seis píxeles cubre cincuenta y "
             "nueve micras en una y ciento diecinueve en la otra.\n"
             "\n"
             "5. De ahí salen dos consecuencias. La escala hay que definirla en micras por píxel y "
             ...
             "resolución confiable en sus metadatos, así que queda fuera del piloto.\n"
             "\n"
             "6. La lámina siguiente pone la cuenta.")
    ...
    notes(s, "1. Acá está la cuenta que relaciona el aumento, el área de tejido y el tamaño del "
             "parche en píxeles.\n"
             "\n"
             "2. La cantidad que manda es las micras por píxel, que es lo que está arriba. El lado "
             "físico de un parche es el número de píxeles por las micras por píxel, y el área es ese "
             "lado al cuadrado.\n"
             "\n"
             "3. Las dos filas del medio son las dos cohortes con el mismo parche de doscientos "
             "veinticuatro píxeles. En la pública, que está a cuarenta aumentos, cubre cincuenta y "
             "dos micras. En la privada, que está a veinte, cubre ciento cuatro. Mismo número de "
             "píxeles y el doble de campo: ése es el sesgo de escala entre cohortes.\n"
             "\n"
             "4. Abajo está cómo se iguala, que es agrandar el parche en la cohorte de más "
             "resolución. Para cubrir en la pública el mismo campo de ciento cuatro micras hacen "
             "falta cuatrocientos cuarenta y ocho píxeles. Doscientos veinticuatro a veinte aumentos "
             "equivalen a cuatrocientos cuarenta y ocho a cuarenta: misma área física, distinta "
             "resolución.\n"
             "\n"
             "5. Por eso las escalas se definen en micras por píxel y no en niveles del archivo. "
             "Antes de elegir cuáles, conviene mirar qué dice la patología.")
    ...
    notes(s, "1. Antes de elegir números me apoyo en la patología, donde hay dos hechos que mandan.\n"
             "\n"
             "2. El primero está arriba: no todas las calcificaciones se ven igual en la tinción de "
             "rutina. Las de tipo uno, de oxalato de calcio, son casi invisibles en campo claro y "
             "solo aparecen con luz polarizada. El modelo trabaja sobre la tinción normal, así que "
             "está ciego a ellas a cualquier escala, y eso pone un techo, sobre todo en tejido no "
             "neoplásico.\n"
             "\n"
             "3. Las de tipo dos, de fosfato de calcio, son basófilas, se ven bien y muestran los "
             "anillos laminados característicos. Ahí sí la escala puede aportar, y es donde el campo "
             "grueso tiene algo que hacer.\n"
             "\n"
             "4. El segundo hecho es de tamaño, y es el mismo de recién: la calcificación entra en "
             "un parche fino, y el conducto que la aloja, no.\n"
             "\n"
             "5. Abajo están las fuentes en tres columnas: la literatura clínica de la "
             "microcalcificación, los trabajos de multi escala en patología en los que nos apoyamos, "
             "y las referencias del encoder y del protocolo del que sale la etiqueta.\n"
             "\n"
             "6. Con esos dos hechos ya se pueden elegir las escalas.")
    ...
    notes(s, "1. Esta es la decisión concreta, y es donde pido guía.\n"
             "\n"
             "2. La propuesta son dos escalas. Una fina, de ciento doce micras de campo, cerca de "
             "veinte aumentos, que es donde el encoder fue entrenado: ahí se ve la calcificación y "
             "su forma. Y una de contexto, de quinientas doce micras, cerca de cinco aumentos, que "
             "abarca el conducto o el lobulillo, que es literalmente lo que pregunta la etiqueta.\n"
             "\n"
             "3. A la derecha está una región real de una lámina de mama, el mismo centro visto a "
             "las dos escalas. En el campo fino se ve la citología; en el de contexto aparece la "
             "arquitectura glandular, y la caja marca dónde cae el parche fino dentro del grueso.\n"
             "\n"
             "4. La tabla traduce eso a píxeles. Como las cohortes están a distinta resolución, el "
             "tamaño se calcula por lámina para dar en el mismo campo físico: en la pública el "
             "recorte fino son cuatrocientos ochenta y dos píxeles y el de contexto algo más de dos "
             "mil doscientos, y en la privada la mitad en cada caso. Después todo se reescala a "
             "doscientos veinticuatro, que es la entrada que el encoder espera.\n"
             "\n"
             "5. Y la fusión, que está abajo: los dos vectores se promedian en uno solo, así el "
             "agregador queda intacto y la comparación con el modelo actual es limpia.\n"
             "\n"
             "6. Las dos escalas y la fusión por promedio son la propuesta a discutir. Los campos "
             "son ajustables, y es justamente lo que quiero conversar ahora.")
