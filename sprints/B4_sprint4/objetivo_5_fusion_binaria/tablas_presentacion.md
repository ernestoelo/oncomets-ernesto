# Tablas para la presentación — slides por entrenamiento + k-fold + comparativa

> Para copiar/pegar a la presentación. Columnas de resultados dejadas **vacías
> a propósito** (se llenan en la slide siguiente cuando termine el chain
> 4170→4171→4172). Conteos = verdad de campo (CSVs/splits reales, 27-may).

---

## A. ¿Para qué hicimos k-fold? (explicación)
...
**Consecuencia.** Cuando comparamos nuestro 0.808 contra el 0.79 de Sebastián,
ese "+0.018" puede ser **ruido del sorteo**, no una mejora real. No podemos
afirmar nada con un solo número.

**Qué es k-fold.** En vez de un solo sorteo, hacemos **k particiones distintas**
de test (k=5 en las binarias). Entrenamos k veces, cada una con un test
diferente, y obtenemos **k números**. Reportamos:
...
incertidumbre tiene. k-fold es exactamente eso para el AUC.
...
sorteo de test, que es el grande. Por eso la palanca correcta es **k-fold**
(re-sortear el test), no multi-semilla.
...
Punto clave para la slide: *k-fold no mejora el modelo; mejora nuestra
**confianza en el número**. Sin barras de error, "0.808 vs 0.79" no significa
nada.*
...
| 9 | 4170 | Fase 0 varianza — carcinoma | CLAM_MB | binaria | 328 | **5** | ⏳ |  ±  |  ±  |
| 10 | 4170 | Fase 0 varianza — CDIS | CLAM_MB | binaria | 328 | **5** | ⏳ |  ±  |  ±  |
| 11 | 4170 | Fase 0 varianza — tejido | CLAM_MB | binaria | 328 | **5** | ⏳ |  ±  |  ±  |
| 12 | 4171 | Fase 1 — fusionado | CLAM_MB | fusionada | 2814 | **3** | ⏳ |  ±  |  ±  |
| 13 | 4172 | Fase 2 — fusionado | DSMIL | fusionada | 2814 | **3** | ⏳ |  ±  |  ±  |

> ✅ hecho · ⏳ en cola/corriendo · `±` = se reporta media ± desviación (k-fold).

### C.1 — Comparativa contra Sebastián (referencia, su tabla 26-may, test_auc)

| Tarea | Nosotros (4109) | Sebastián | Dataset Sebastián |
|---|---|---|---|
| micro carcinoma invasivo | 0.808 | 0.79 | combined (284) |
| micro CDIS | 0.678 | 0.69 | pth_balance + no_id (~2128) |
| micro tejido no neoplásico | 0.658 | 0.63 | pth_balance (328) |

> En test_auc estamos **en paridad** (|Δ| ≤ 0.03) — dentro del ruido de 1 sorteo.
> Fase 0 (k-fold) dará las barras de error para confirmarlo.

---

## D. Notas para llenar resultados (cuando termine el chain)

- **Fila 9-11 (Fase 0):** de `results/obj5_varianza_<tejido>/.../summary.csv`
  (test_auc por fold → media ± std) + balanced_acc desde los `split_*_results.pkl`.
- **Fila 12-13 (Fase 1/2):** de `results/obj5_fase1_clam_fusionado/clam_presencia_f*_s1/test_metrics.json`
  y `results/obj5_fase2_dsmil_fusionado/dsmil_presencia_f*_s1/test_metrics.json`
...
- La comparación clave de la presentación: **¿DSMIL (fila 13) supera a CLAM
  (fila 12) sobre el fusionado, con barras de error que NO se solapan?**
