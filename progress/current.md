...
| 1b | **mammoth en patrón arquitectónico (4 binarias) + invasión (3-clase)** k=5 | **RE-LANZADO job 4243 desde `main`** (GROUP=patron, 3-jun); el job 4241 había crasheado por branch-switch. Invasión = 2ª ola. `objetivo_2_mammoth_patron_invasion/` |
...
- **mammoth patrón+invasión (`objetivo_2_mammoth_patron_invasion/`) — RE-LANZADO**:
  job **4243** (GROUP=patron, 4 binarias × k=5 × 2 brazos = 40 runs, ~13h), lanzado
  3-jun **desde `main`** (con CSVs+splits commiteados) **sin cambiar de rama durante
  el job**. El intento previo (job **4241**, 2-jun) completó 1/40 y murió porque un
  `git checkout` en el working-tree COMPARTIDO borró `data/csv_new_tasks/` →
  `FileNotFoundError` en fold 1 ([[working-tree-compartido-job-en-curso]], workaround
  H de CLAUDE.md). Monitoreo: `tail -f logs/eg_mammoth_patinv_4243.out` · `squeue -j 4243`.
  Invasión (3-clase, ~25h) NO lanzada (2ª ola deliberada, con OK + cortesía).
