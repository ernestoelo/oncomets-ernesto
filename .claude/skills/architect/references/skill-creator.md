...
- **Frontmatter** (YAML): `name` and `description` are the core fields. Description determines when Claude activates the skill — include explicit trigger phrases in quotes and topic keywords. Descriptions longer than 250 characters are truncated in the listing. Additional optional fields: `disable-model-invocation`, `user-invocable`, `allowed-tools`, `context`, `agent`, `model`, `effort`, `hooks`, `paths`, `argument-hint`, `shell`. See SKILL.md for the full reference table.
...
### Step 3: Scaffolding the Skill

Run `scripts/skill_scaffold.py <skill-name> "<description>"` to scaffold a new skill. This creates SKILL.md, scripts/, references/, and assets/ with templates.
